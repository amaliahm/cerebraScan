...
function wirePauseOnHover(reel, track) {
  const pauseSel = reel.getAttribute("data-team-reel-pause-on");
  if (!pauseSel) return;
...
export function initteamReel() {
  document.querySelectorAll("[data-team-reel]").forEach((reel) => {
    if (!(reel instanceof HTMLElement)) return;
    const track = reel.querySelector(".team-reel__track");
    if (!(track instanceof HTMLElement)) return;
    if (track.dataset.teamReelHydrated === "1") return;
    track.dataset.teamReelHydrated = "1";

    const icons = parseIcons(reel.dataset.teamReelIcons);

    /* Duplicate icon sequence so CSS animation can loop without a visible seam. */
    for (let pass = 0; pass < 2; pass += 1) {
      icons.forEach((src) => {
        const item = document.createElement("span");
        item.className = "team-reel__item";
